...
const createHoliday = async (req, res) => {
    ...
    activityList, poiList, taskList} = req.body
    ...
    // add doc to db
    try {
        const holiday = await Holiday.create({title, description, departureDate, returnDate, travelList, accommodationList,
        activityList, poiList, taskList})
        res.status(200).json(holiday)
    } catch (error) {
